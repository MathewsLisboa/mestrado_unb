...
sink("Results_Scenario2.txt")

names_results1 <- c("Estimates_Scenario2_n30", "Estimates_Scenario2_n60", "Estimates_Scenario2_n100", "Estimates_Scenario2_n500")
names_results2 <- c("SE_Estimates_Scenario2_n30", "SE_Estimates_Scenario2_n60", "SE_Estimates_Scenario2_n100", "SE_Estimates_Scenario2_n500")

########################################################################
#######################Global Variables#################################
########################################################################
 
VN <- c(30, 60, 100, 200) #Sample sizes
VBETA <- c(4, 2, 3) #true beta values
VSIGMA <- 1 #true sigma value
VXI <- 0.5 #true xi value
VDELTA <- 1 #true delta value

VTHETA <- c(VBETA, VSIGMA, VXI, VDELTA) # true theta values
NREP <- 100 #number of Monte Carlo replicates
kk1 <- length(VBETA)
