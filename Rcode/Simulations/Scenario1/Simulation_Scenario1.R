...
names_results1 <- c("Estimates_Scenario1_n30", "Estimates_Scenario1_n60", "Estimates_Scenario1_n100", "Estimates_Scenario1_n500")
names_results2 <- c("SE_Estimates_Scenario1_n30", "SE_Estimates_Scenario1_n60", "SE_Estimates_Scenario1_n100", "SE_Estimates_Scenario1_n500")

########################################################################
#######################Global Variables#################################
########################################################################
 
VN <- c(30, 60, 100, 500) #Sample sizes
...
if(perc == 0.25 || perc == 0.5 || perc == 0.75 || perc ==1) cat("Perc. Replic. MC =",perc*100,"%","\n")
 ...
 # regression fit
 fitMLE   <- MLE_BGEV(y=y, X=X, method="BFGS")
